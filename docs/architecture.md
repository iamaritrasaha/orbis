...
The dashboard creates bounded, read-only standard-thread workers for source snapshots, update inventories, searches, and plans. They communicate through a channel; the event loop never waits on provider I/O. A selected package action requests an `OperationPlan`, displays a review dossier, and rejects blocked plans. Confirmation yields an execution intent from the session. Only after `ratatui::run` returns and restores the terminal does the outer controller authorize administrator access, when required by the exact plan. A new session presents worker execution through the same typed executor and history lifecycle as the plain CLI. Interactive authorization is an explicit call available only to the outer controller and plain command orchestration; executor execution methods only check cached credentials with noninteractive sudo. Upgrade plans retain their safety revalidation after authorization. Explicit interactive commands use the same review and live execution view. Maintenance source activity is carried by typed progress events; provider output is bounded and presentation-only, with a separate details mode and best-effort APT source activity. Ctrl-C during provider execution defers exit until completion, preserving the history lifecycle. Update review is likewise a read-only maintenance plan.
