...
Normal commands use `render/transient.rs`: a bounded reveal and scrollback-preserving inline output. Bare `orbis` opens a task-first transient launcher. Only explicit `orbis dashboard`/`orbis ui` enters the persistent TUI; `--plain`, `TERM=dumb`, pipes, and JSON stay deterministic and never enter alternate-screen mode. `ratatui::run` owns the Crossterm raw-mode/alternate-screen lifecycle for that explicit mode and restores the terminal on normal exit or returned initialization/draw errors.

The dashboard creates bounded, read-only standard-thread workers for source snapshots, update inventories, searches, and plans. They communicate through a channel; the event loop never waits on provider I/O. A selected package action requests an `OperationPlan`, displays a review dossier, and rejects blocked plans. Confirmation yields an execution intent from the session. Only after `ratatui::run` returns and restores the terminal does the outer controller authorize administrator access, when required by the exact plan. The transient CLI presents the same typed plan/executor/history lifecycle without a full-screen session; authorization is an explicit call available only to the outer controller and plain command orchestration. Upgrade plans retain their safety revalidation after authorization. Maintenance source activity is carried by typed progress events; provider output is bounded and presentation-only. Ctrl-C during provider execution defers exit until completion, preserving the history lifecycle.
